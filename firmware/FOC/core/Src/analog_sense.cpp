...
{
    if (count_ < 4096) {
        offset_.fU += (float)(*addr_u_) * fcc_;
        offset_.fV += (float)(*addr_v_) * fcc_;
        offset_.fW += (float)(*addr_w_) * fcc_;
        count_++;
    } else if (count_ == 4096) {
        offset_.fU /= 4096.0f;
        offset_.fV /= 4096.0f;
...
{
    if (cfg.addr_u != nullptr &&
        cfg.addr_v != nullptr &&
        cfg.addr_w != nullptr) {
        float adc_rate = cfg.adc_trans_volt;
...
void InlineCurrentSense::Update()
{
    if (resource_init_ && resource_init_) {
        currents_.fU = (float)(*addr_u_) * fcc_ - offset_.fU;
        currents_.fV = (float)(*addr_v_) * fcc_ - offset_.fV;
        currents_.fW = (float)(*addr_w_) * fcc_ - offset_.fW;
...
LowsideCurrentSense::LowsideCurrentSense(const LowsideCurrentSense::SenseConfig &cfg)
{
    if (cfg.addr_u != nullptr &&
        cfg.addr_v != nullptr &&
        cfg.addr_w != nullptr) {
        fcc_           = cfg.adc_trans_volt / cfg.resistance / cfg.gain;
        addr_u_        = cfg.addr_u;
        addr_v_        = cfg.addr_v;
        addr_w_        = cfg.addr_w;
        resource_init_ = true;
