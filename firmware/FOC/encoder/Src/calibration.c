...
void calibrate_mt_encoder(float vd_set, float pos)
{
	...
	switch(cali_flag)
	{
		...
		case 1:
			...
			{
				if( e_theta < 0.55f*M_PI)
					end_dir_pos = enc_para.pos;
		...
		case 6:
		{
			// TODO_1: The reference value of the first index here is not 0, but the value of the last index is 0, which needs to be corrected.
			for (i=0; i < N; i++)
			{
				p_error_array[i] = 0.5f * (p_error_array[i] + p_error_array[2*N-i-1]);
...
//							error_filt[i] += p_error_array[ind] / (float)window;
...
//					mean += error_filt[i] / N;
		...
		case 8:
		{
			...
			raw_offset = (p_raw[0]+p_raw[1])*0.5f; //Insensitive to errors in this direction, so 2 points is plenty
			for (int i = 0; i < n_lut; i++)
			{                                          // build lookup table
				ind = (raw_offset >> 6) + i;
				if(ind > (n_lut - 1))
				 ind -= n_lut;
				//lut[ind] = (error[i] - mean) * 2607.5945876176f;  // 16384/two_pi
				//printf("%f\r\n", lut[ind]);
			}
//			printf("\r\n");
			cali_flag = 9;
